...
export async function GET() {
  try {
    ...
    /** 장소들 */
    const locations: IIFavoriteLocation[] | null =
      await FavoriteLocation.find();
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // KakaoMapId가 있을 시
    if (kakaoMapId)
      deleteResult = await FavoriteLocation.deleteOne({ kakaoMapId });
    // 주소로 삭제 시
    else if (addressName)
      deleteResult = await FavoriteLocation.deleteOne({ addressName });
    // 일치하는 결과가 없을 시
    else
      return NextResponse.json(
        { msg: "삭제할 정보를 찾을 수 없습니다." },
        { status: 400 }
      );
