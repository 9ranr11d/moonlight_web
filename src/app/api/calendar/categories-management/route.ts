...
export async function GET() {
  try {
    ...
    /** 카테고리들 */
    const categories: IIScheduleCategory[] | null =
      await ScheduleCategory.find();
...
export async function PUT(req: NextRequest) {
  try {
    ...
    /** 카테고리 정보 갱신 */
    const updatedScheduleCategory = await ScheduleCategory.findByIdAndUpdate(
      _id,
      { color, title },
      { new: true }
    );
